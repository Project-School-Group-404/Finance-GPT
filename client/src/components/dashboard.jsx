        ...
        if (type === 'user') {
            ...
            try {
                ...
                if (fileArray.length > 0) {
                    // Use /invoke_with_files endpoint for file uploads
                    ...
                    response = await fetch("http://localhost:8000/invoke_with_files", {
                        method: "POST",
                        body: formData
                    });
                } else {
                    // Use regular /invoke endpoint for text-only messages
                    ...
                    response = await fetch("http://localhost:8000/invoke", {
                        method: "POST",
                        headers: {
                ...
                if (response.ok) {
                    ...
                    const botResponse = {
                        id: Date.now() + 1,
                        text: data.response, // FastAPI returns "response" not "reply"
                        file: null,
                        type: 'bot',
                        timestamp: new Date()
                    };
                    ...
                    try {
                        ...
                        const saveResponse = await fetch("http://localhost:3000/api/chat/ai", {
                            ...
                            body: JSON.stringify({
                                ...
                                assistantReply: data.response, // Updated to match FastAPI response
                                userId: user.id,
